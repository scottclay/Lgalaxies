...
#for loop in range(0,1):
	try: 
		...
		try: 
			ax[loop] = cloudpickle.load(open('./pkl_hists/DTG_stellar_z'+str(loop)+'.pkl','rb'))
		except IOError:
		    	pass
	except IOError:
		...
		from fit_scatter import fit_median
		median, bin_centres, per_50,per_16,per_84,per_25,per_75 = fit_median(SM,DTG,30)
		...
		try: 
			...
			if loop == 0: 
				hb = plt.hexbin(SM,DTG,gridsize=150,bins='log',mincnt=5,cmap='gist_gray')
				min = hb.norm.vmin
				max = hb.norm.vmax
				normalize = matplotlib.colors.Normalize(vmin=min, vmax=max)
				print(min,max)
			else:
				plt.hexbin(SM,DTG,gridsize=150,bins='log',mincnt=5,cmap='gist_gray',norm=normalize)
			import pickle   
	
			fout = open('./pkl_hists/DTG_stellar_z'+str(loop)+'.pkl','wb')
