...
for loop in range(0,9):
    try: 
        bin_centres,median,per_50,per_16,per_84,per_25,per_75 = np.loadtxt('./binned_data/OX_DTG_z'+str(loop)+'.txt',unpack=True,comments='#')
        ax[loop] = cloudpickle.load(open('./pkl_hists/OX_DTG_z'+str(loop)+'.pkl','rb'))
    except IOError:
...
pylab.savefig('./figs/DTG_oxygen.eps', bbox_inches=0)
plt.close()
